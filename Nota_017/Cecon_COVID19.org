...
#+begin_src sh :dir /HDD/Cecon_Atividade/Nota_016/ :exports none :eval no
...
#+begin_src shell :dir /HDD/Cecon_Atividade/Nota_016/raw/Granulares/ :exports none :eval no
...
df.rolling(7).mean().plot(
    title="Relatório de mobilidade do google - Brasil\nSemana Móvel",
    ax=ax,
    lw=2,
)
ax.axvline(
...
sp = sp[sp["sub region 1"] == "State of São Paulo"]
...
sp.rolling(7).mean().plot(
    title="Relatório de mobilidade do google - Estado de São Paulo\nSemana Móvel",
    ax=ax,
    lw=2,
)
ax.axvline(
...
cidade = cidade[cidade["sub region 1"] == "State of São Paulo"]
cidade = cidade[cidade["sub region 2"] == "São Paulo"]
...
cidade.rolling(7).mean().plot(
    title="Relatório de mobilidade do google - Cidade de São Paulo\nSemana Móvel",
    ax=ax,
    lw=2,
)
ax.axvline(
...
sns.regplot(
    data=df,
    y="2020:H1 Erro da previsão do PIB (%)",
    x="Rigidez do Lockdown (índice, média de 2020:H1)",
    ax=ax,
)
# ax.legend()
ax2 = plt.axes([0.135, 0.135, 0.2, 0.2])
...
label_point(
    df["Rigidez do Lockdown (índice, média de 2020:H1)"],
    df["2020:H1 Erro da previsão do PIB (%)"],
    df["Country"],
    ax,
)
sns.despine()
...
# df = df.transpose()
df.set_index(0, inplace=True)
df.index.name = "Economias"
df = df.rename(columns=df.iloc[0]).drop("Economias")
for col in df:
    df[col] = [x.replace(",", ".") for x in df[col]]
df = df.apply(pd.to_numeric, axis=1)
...
ax.axhline(y=0, ls="-", color="black")
ax.legend()
ax2 = plt.axes([0.135, 0.635, 0.2, 0.2])
ax2.imshow(logo, aspect="auto", zorder=0, alpha=0.5)
ax2.axis("off")
...
** Download :noexport:
#+begin_src shell :dir /HDD/Cecon_Atividade/Nota_016/raw/OECD/ :exports none :eval no
...
ax[0].axvline(
    x=corona_sp, color="black", ls="--", lw=1, label="Início isolamento em SP"
)
...
* Novos casos x Restrição de mobilidade por tipo de isolamento :noexport:
** TODOs :noexport:
...
df <- read_csv(
  ...
  mutate(C6 = case_when(
           ...
           C6 == 2 ~ "Proibido sair\n(Salvo exceções)",
           C6 == 3 ~ "Proibido sair"),
    C6 = fct_relevel(C6, "Sem restrição", "Recomendado ficar\nem casa", "Proibido sair\n(Salvo exceções)", "Proibido sair"))
...
df <-read_csv(
  ...
  mutate(C6 = factor(C6)) %>%
  mutate(C6 = fct_relevel(C6, "Dados indisponíveis", "Sem restrição", "Recomendado ficar\nem casa", "Proibido sair\n(Salvo exceções)", "Proibido sair")) %>%
...
df <- df %>%
  ...
  mutate(Isolamento = case_when(
     stayhome_growth_after >= 0 & C6 == "Algumas exceções\npara sair" |  C6 == "Proibido sair" ~ "Rígido", # Queda da mobilidade junto da imposição
     stayhome_growth_after > 0 & Policy_change > 0  &  C6 == "Recomendado ficar\nem casa" | C6 == "Sem restrição" ~ "Brando",
     stayhome_growth_after > 0 & Policy_change <=0 & C6 != "Proibido sair" ~ "Voluntário", # Queda da mobilidade sem restrição
...
** Número de casos acumulados por estados
...
** Número de mortes acumuadas por estados
...
df %>% ggplot(aes(x=date)) +
  ...
  labs(
    x = ""
      ...
      scale_y_continuous(
        name = "Novos casos por milhão de habitantes",
        sec.axis = sec_axis( trans=~.*rescale, name="Novos mortes por milhão de habitantes")
      ) +
      theme(
...
for(estado in estados){
  ...
  p2 <- tmp_long %>%
    ...
    scale_color_manual(
      ...
      breaks=c("Dados indisponíveis", "Sem restrição", "Recomendado ficar\nem casa", "Proibido sair\n(Salvo exceções)", "Proibido sair"),
      name=policy_name) +
    theme_minimal() +
    theme(
      legend.position = "right",
...
** Gráficos de mobilidade e política por estados para indicar quando houve medida adotada
...
df <- read_csv(
  ...
  mutate(C6_num = case_when(
           ## C6 == "Dados indisponíveis" ~ NA_integer_,
           C6 == "Sem restrição" ~ 0,
