...
#+INCLUDE: Infos.org
#+EXCLUDE_TAGS: Notes
#+PROPERTY: header-args python :results output drawer replace :exports results :eval never-export :session "Cecon"
...
#+BEGIN_SRC sh :dir ./raw/ContasNacionais/ :exports none
...
#df["PIB"].pct_change().tail(12).plot(ax = ax, kind = 'line', legend = True, color = 'black')
df[Servicos].drop(["Total Servicos"]).diff().apply(lambda x: x/(df["Total Servicos"].shift())).tail(4).plot(
    kind = 'bar',
    stacked = True,
    ax = ax,
    color = (
...
* Base de dados FMI (Fiscal Monitor e WEO)
