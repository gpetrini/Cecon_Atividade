...
(python3:8919): Gtk-[1;33mWARNING[0m **: [34m16:43:26.492[0m: Theme parsing error: gtk.css:1:117: Failed to import: Erro ao abrir arquivo /home/gpetrini/.local/share/gnome-shell/extensions/unite@hardpixel.eu/styles/buttons-right-always.css: Arquivo ou diretório inexistente
...
df <- read_csv(
  ...
  filter(iso_code == "BRA") %>%
  select(date, Residential, C1, C1_Flag, C2, C2_Flag, C6, C6_Flag, Pico) %>%
  mutate(`Ficar em casa` = case_when(
           C6 == 0 ~ "Sem medida",
           C6 == 1 ~ "Recomendado ficar em casa",
           C6 == 2 ~ "Algumas exceções podem sair",
           C6 == 3 ~ "Sem exceções para sair",
           C6 == NA ~ "Sem dados")
    ## C6 = fct_relevel(C6, "Sem dados", "Sem medida", "Recomendado ficar em casa", "Algumas exceções", "Pouca Exceções"), 
    ## C6_bin = if_else(C6 == "Sem medida", 0, 1)
  ) 
...
p1 <- df %>% ggplot(aes(x = date, y = Residential)) + geom_line() + 
  geom_vline(
    data = df_inf, 
    mapping = aes(xintercept = vline),
    color = "black"
...
p2 <- df %>% ggplot(aes(x = date, y = 1, color=`Ficar em casa`) ) + 
  geom_line(   size = 7   ) + scale_y_discrete(labels = abbreviate) +
  scale_color_manual(values=c("#9F9F92", "#E67960", "#AB2323", "#c00000")) + 
  theme_minimal() +
...
cowplot::plot_grid(p1, p2, nrow = 2, rel_heights = c(3,1), align="v", axis="lr")
...
2: Removed 6 row(s) containing missing values (geom_path).
:end:
