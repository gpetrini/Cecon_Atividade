...
(python3:52522): Gtk-[1;33mWARNING[0m **: [34m15:36:54.681[0m: Theme parsing error: gtk.css:1:117: Failed to import: Erro ao abrir arquivo /home/gpetrini/.local/share/gnome-shell/extensions/unite@hardpixel.eu/styles/buttons-right-always.css: Arquivo ou diretório inexistente
:end:
** R
...
df <- df %>%
  ...
  ungroup() %>%
  mutate(Isolamento_Strigency = case_when(
    Isolamento_14 == "Imposto" & Strigency_type == "High" ~ "Rígido",
    Isolamento_14 == "Imposto" & Strigency_type == "Low" ~ "Brando",
    Isolamento_14 == "Imposto" & Strigency_type == "Medium" ~ "Médio",
    Isolamento_14 != "Imposto" ~ Isolamento_14 %>% as.character()
  )) %>%
  mutate(Isolamento_Strigency = as.factor(Isolamento_Strigency))
...
** Novos casos por milhão x Fase pandemia (low and high strigency)
#+begin_src R :results value file :var figname="Casos_Policy_LowHigh"

name = paste0('./figs/COVID/', figname, ".pdf")

df %>%
  drop_na(Policy_change, Onda, cases_growth, Isolamento_14) %>%
  ggplot(aes(y=Onda, x= cases_growth, fill=Isolamento_Strigency)) +
  geom_vline(xintercept=0, color="black") +
  geom_boxplot(outlier.shape = NA, outlier.color=NA, outlier.alpha=1) +
  labs(
    y = "Fase da Pandemia",
...
** Novos casos por milhão x Fase pandemia (low strigency)
#+begin_src R :results value file :var figname="Casos_Policy_Low"
...
#+RESULTS:
:results:
[[file:./figs/COVID/Casos_Policy_Low.pdf]]
:end:

** Novos casos por milhão x Fase pandemia (high strigency)
#+begin_src R :results value file :var figname="Casos_Policy_High"
...
#+RESULTS:
:results:
[[file:./figs/COVID/Casos_Policy_High.pdf]]
:end:
