...
(python3:53122): Gtk-[1;33mWARNING[0m **: [34m15:31:11.985[0m: Theme parsing error: gtk.css:1:117: Failed to import: Erro ao abrir arquivo /home/gpetrini/.local/share/gnome-shell/extensions/unite@hardpixel.eu/styles/buttons-right-always.css: Arquivo ou diretório inexistente
:end:
** R
...
df <-read_csv(
  ...
  mutate(Policy = case_when(
           ...
           Policy_change == 0 ~ "Sem novas medidas"
         )) %>%
...
** Econômico: Pandemia $\Rightarrow$ Isolamento
...
*** Novos casos por milhão > 10 para todos os dias
...
df %>%
  ...
  labs(
    x = "Variação imposição de restrição",
    y = ylab
  ) + 
...
:end:
*** Novos casos por milhão > 10 apenas nos dias que houve mudança na política de restrição
...
*** Novos casos por milhão > 100 para todos os dias
...
:end:
*** Novos casos por milhão > 100 apenas nos dias que houve mudança na política de restrição
...
** Epidemiológico: Isolamento $\Rightarrow$ Pandemia

#+begin_src R :exports none
ylab = "Taxa de crescimento de casos após 14 dias"
#+end_src
